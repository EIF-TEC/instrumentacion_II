...
\rhead{Instrumentación II \\ Tarea \#2  \\ Entrega: Semana 10} 
...
\begin{itemize}
    \item Tener cinco estados definidos como:
        \begin{itemize}
            ...
            \item \emph{hello}: estado que concatena letra por letra de la palabra \emph{hello} en un string que inicialmente esta vació, debe aparecer una nueva letra cada segundo, al finalizar la palabra pasa a estado \emph{space}
            \item \emph{space}: estado que concatena un espacio vacío al string y espera por 5 segundos, pasa al estado \emph{world}
            \item \emph{world}: estado que concatena letra por letra la palabra \emph{world} en el string, al finalizar la palabra pasa a estado \emph{end}
